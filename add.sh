...
for file in "$@"; do
	...
	for commit in $latest; do
		...
		tar -zxf ".jsv/commits/$commit" -p -C ".jsv/temp-commit"
		### temp-commit has DIRECTORY..FILE
		commitFiles=$(ls ".jsv/temp-commit")
		for commitFile in $commitFiles; do
			...
			if [ "$commitFileName" == "" ]; then
				commitFileName="$commitFileDirectory"
				if [[ "$commitFileName" == $(basename "$file") ]]; then
					patch -R ".jsv/current.txt" ".jsv/temp-commit/$commitFileName"
				fi
			else
